...
// searchHit is one result: a name to show and where it goes.
...
// handleSearchPage serves both the search page and the command palette's
...
func (s *Server) handleSearchPage(w http.ResponseWriter, r *http.Request) {
	...
	results, err := s.search(r.Context(), query, user.IsAdmin, s.translatorFor(w, r))
	if err != nil {
...
func (s *Server) search(ctx context.Context, query string, isAdmin bool, t translator) (searchResults, error) {
	...
	for _, p := range players {
		...
		if strings.Contains(strings.ToLower(p.Name), needle) || strings.Contains(strings.ToLower(p.Slug), needle) {
			results.Players = append(results.Players, searchHit{Label: p.Name, Href: "/p/" + p.Slug})
		}
	}

	for _, d := range s.searchDestinations(isAdmin, t) {
...
// searchDestinations lists the pages a search can find: every nav view, the
// reader's own settings, and — for an admin — the admin area's four
// screens. Built fresh per request rather than once at startup, because the
// labels are localized and an admin's list is longer than everyone else's.
func (s *Server) searchDestinations(isAdmin bool, t translator) []searchHit {
	...
	destinations = append(destinations, searchHit{Label: t.T("nav.settings"), Href: "/settings"})

	if isAdmin {
		destinations = append(destinations,
			searchHit{Label: t.T("admin.players.title"), Href: "/admin/players"},
			searchHit{Label: t.T("pending.title"), Href: "/admin/pending"},
			searchHit{Label: t.T("activity.title"), Href: "/admin/activity"},
			searchHit{Label: t.T("diag.title"), Href: "/admin/diagnostics"},
		)
	}
	return destinations
}
