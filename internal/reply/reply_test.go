...
func newAnswerer(t *testing.T, db *sql.DB, interp Interpreter) (func(context.Context, string, string, string, []string) error, *collector) {
	t.Helper()
	cats, err := i18n.Load()
	if err != nil {
...
func TestAFailingModelIsReportedAndApologised(t *testing.T) {
	...
	err := answer(context.Background(), senderUUID, "who leads?", "", nil)
